...
class NearAxisQuasiSymmetryObjective():
    def __init__(self, stellarators, mas, iota_target, eta_bar=-2.25,
        ...
        if mode in ["deterministic", "stochastic"]:
            ...
            if not self.freezeCoils: 
                self.x0 = np.concatenate((eta_bar_cat, ma_dofs_cat, stellarator_cat, self.stellarator_group[0].get_dofs()))
            else:
                self.x0 = np.concatenate((eta_bar_cat, ma_dofs_cat, stellarator_cat))
        elif mode[0:4] == "cvar":
    ...
    def set_dofs(self, x):
        ...
        def split_list(alist, wanted_parts=self.num_stellarators):  
    ...
    def update(self, x):
        ...
        if not self.freezeCoils:
            ...
            self.drescoil += self.stellarator_group[0].reduce_coefficient_derivatives([
            ...
            self.res2 = 0
        self.res3    = np.sum([0.5 * (1/magnetic_axis_length_target)**2 * (J_axis_length[i].J() - magnetic_axis_length_target)**2 for i in self.stellList]) 
        self.dresma += np.concatenate(([(1/magnetic_axis_length_target)**2 * (J_axis_length[i].J()-magnetic_axis_length_target) * J_axis_length[i].dJ_by_dcoefficients() for i in self.stellList]))
        ...
        if not self.tanMap:
            ...
            if not self.constrained:
                ...
                self.drescoil    += np.concatenate([self.iota_weight * (1/iota_target[i]**2) * ((tanMap_iota[i] - iota_target[i]) * tanMap_group[i].d_iota_dcoilcoeffs() + 0.5 * self.tanMap_resAxis_additionalWeight*tanMap_group[i].d_res_axis_d_coil_coeffs()) for i in self.stellList])
                ...
                self.drescoil    += np.concatenate([self.iota_weight * (1/iota_target[i]**2) * (tanMap_iota[i] - iota_target[i]) * tanMap_group[i].d_iota_dcoilcoeffs() for i in self.stellList])
        ...
        if self.qfm_weight > self.ignore_tol:
            ...
            self.drescoil += np.concatenate([self.qfm_weight*self.qfm_group[i].d_qfm_metric_d_coil_coeffs() for i in self.stellList])
    ...
    def callback(self, x, verbose=True):
        ...
        if not self.freezeCoils:
            ...
            self.dJvals.append((
                norm(self.dres), norm(self.dresetabar), norm(self.dresma), norm(self.drescurrent)
            ))
        ...
        if not self.freezeCoils:
            ...
            info(f"Objective gradients:     {norm(self.dresetabar):.6e}, {norm(self.dresma):.6e}, {norm(self.drescurrent):.6e}")
